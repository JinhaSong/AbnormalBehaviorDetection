...
def draw_points(image, points, color_palette='tab20', palette_samples=16, prob_thresh=0.5):
    """
    ...
    Args:
        ...
        palette_samples: number of different colors sampled from the `color_palette`
            Default: 16
        prob_thresh: only points with a confidence higher than this threshold will be drawn. Range: [0, 1]
            Default: 0.5
...
def draw_skeleton(image, points, skeleton, color_palette='Set2', palette_samples=8, person_index=0,
    ...
    for i, joint in enumerate(skeleton):
        ...
        if pt1[2] > prob_thresh and pt2[2] > prob_thresh:
            image = cv2.line(
