...
def create_contrastive_h5(output_path, normal_folders, abnormal_folders, num_frames, num_pos, num_neg, csv_path, in_channels):
    video_list = []
    contrastive_pairs = []
    normal_indices = []
    abnormal_indices = []
    ...
    with h5py.File(output_path, 'w') as h5f:
        ...
        for i, folder in enumerate(tqdm(normal_folders, desc="Adding normal videos")):
            ...
            video_list.append((i, 0, os.path.basename(folder)))
        ...
        for i, folder in enumerate(tqdm(abnormal_folders, desc="Adding abnormal videos")):
            ...
            video_list.append((idx, 1, os.path.basename(folder)))
    ...
    with open(os.path.join(csv_path, output_path.replace(".h5", "_contrastive_pairs.csv")), 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
...
def create_triplet_h5(output_path, normal_folders, abnormal_folders, num_frames, num_pos, num_neg, csv_path, in_channels):
    ...
    with h5py.File(output_path, 'w') as h5f:
        ...
        for i, folder in enumerate(tqdm(abnormal_folders, desc="Adding abnormal videos")):
            ...
            video_list.append((i + abnormal_start_idx, 1, os.path.basename(folder)))
    ...
    with open(os.path.join(csv_path, output_path.replace(".h5", "video_list.csv")), 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Index', 'Class', 'Video'])
        writer.writerows(video_list)
...
def main():
    ...
    parser.add_argument('--learning-type', choices=["triplet", "contrastive"], required=True, help="Type of learning")
    ...
    for count in frame_counts:
        ...
        if args.learning_type == "triplet":
            ...
            train_contrastive_h5_path = os.path.join(args.output_dir, f'train_contrastive_heatmap_f{count}.h5')
            create_contrastive_h5(train_contrastive_h5_path, train_video_folders_normal, train_video_folders_abnormal, count, args.num_pos, args.num_neg, args.output_dir, in_channels)

            test_contrastive_h5_path = os.path.join(args.output_dir, f'test_contrastive_heatmap_f{count}.h5')
            create_contrastive_h5(test_contrastive_h5_path, test_video_folders_normal, test_video_folders_abnormal, count, args.num_pos, args.num_neg, args.output_dir, in_channels)
